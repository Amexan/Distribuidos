...
app.get("/api/paciente/:id/medicacion", function(req, res){
    ...
    if(lista_Paciente.length != 0){
        ...
        if(lista_Medicacion.length !=0){
            ...
            if(lista_Medicacion_Segun_Paciente.length != 0){
                ...
                res.status(404).json("Ninguna Medicación asignada a dicho Paciente!!");
            }
        }
        else{
            res.status(204).json("La lista de Pacientes esta vacía!!") 
...
var id_Paciente = req.params.id;
    if(lista_Medicacion != 0){
        if(lista_Paciente.length != 0){
            for (var i=0; i<lista_Medicacion.length; i++){
                if(lista_Medicacion[i].paciente <= contador_Paciente && lista_Medicacion[i].paciente>0){ // el paciente debe existir
                    ...
                    if(body_Medicacion.medicamento != "" && body_Medicacion.paciente != "" && body_Medicacion.fecha_Asignacion != "" && body_Medicacion.dosis != "" && body_Medicacion.tomas != "" ){
                        if(body_Medicacion.medicamento> 0 && body_Medicacion.medicamento<=contador_Medicacion){
                            ...
                            return res.status(201).json(lista_Medicacion);
                            
                        }else{
                            return res.status(404).json("Medicamento No Existe");}
                    }else{
                        return res.status(401).json("ERROR! Introduce datos");}
                }else{
        ...
        return res.status(204).json("Lista Medicación vacía")
    }
});
